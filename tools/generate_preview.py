...
def generate_all_previews():
    ...
    # 步骤1：JSON -> DOT（使用自动模式）
    success1 = run_command("python tools/json_to_dot.py --auto", "步骤1：生成DOT文件")
...
def generate_specific_preview(campaign_name, story_name):
    """生成特定剧情的预览文件"""
    base_dir = Path(__file__).parent.parent
    output_dir = base_dir / "output" / campaign_name
    ...
    print(f"处理剧情：{campaign_name}/{story_name}")
...
def main():
    if len(sys.argv) == 1:
        ...
        if not story_name.endswith('.json'):
            story_name += '.json'
        story_name = story_name[:-5]  # 移除.json扩展名
        
        generate_specific_preview(campaign_name, story_name)
    else:
        print("用法：")
        print("  python generate_preview.py                    # 处理所有剧情文件")
        print("  python generate_preview.py 跑团名 剧情名      # 处理特定剧情")
        sys.exit(1)
